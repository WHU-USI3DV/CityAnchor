...
class ReasonSegDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        SCANREFER_TRAIN = json.load(open(os.path.join("./CityAnchor_Release_Data/meta_data/UrbanBIS_Refer_train_all_v1.json"))) 
        SCANREFER_VAL = json.load(open(os.path.join("./CityAnchor_Release_Data/meta_data/UrbanBIS-Refer_val_NO.json"))) 
    ...
    def __getitem__(self, idx):
        ## For original llava if training ROI
        ...
        try:
            ...
            image = self.transform.apply_image(np.zeros((3,3,3)))  # preprocess image for sam (No need for stage 2)
        ...
        if object_id > 0:
            try:
                ...
                prompt_object_category = " Additional information is: The class of this object in image belongs to '" + category.lower() + "'.<end> Query is: " # No need
            except IndexError:
                prompt_object_category = " Additional information is: The class of this object in image belongs to '" + object_name + "'.<end> Query is: " # No need
        else: 
            prompt_object_category = " Additional information is: The class of this object in image belongs to '" + object_name + "'.<end> Query is: " # No need

        query = prompt_object_category + query
