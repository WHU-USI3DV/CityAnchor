...
def main(args):
    ...
    import json
    SCANREFER_TRAIN = json.load(open(os.path.join("./CityAnchor_Release_Data/meta_data/UrbanBIS_Refer_train_all_v1.json"))) # Training samples
    SCANREFER_VAL = json.load(open("./CityAnchor_Release_Data/meta_data/UrbanBIS-Refer_val_NO.json")) # Testing samples
    ...
    file_path = "./vis_output/UrbanBIS-Refer-Val.txt" # Changing this to record grounding results
    ...
    for i in range(len(scanrefer)):
        ...
        scene_id_converted = scene_id.replace("_", "/")

        feat_file = os.path.join("./CityAnchor_Release_Data/data_urbanbis/UrbanBIS_feat", scene_id_converted + "_feat.json")
        bbox_file = os.path.join("./CityAnchor_Release_Data/data_urbanbis/UrbanBIS_bbox", scene_id_converted + "_bbox.json")
        raster_file = os.path.join("./CityAnchor_Release_Data/data_urbanbis/UrbanBIS_map", scene_id_converted + ".tif")
        pc_file = os.path.join("./CityAnchor_Release_Data/data_urbanbis/UrbanBIS_Inst", scene_id_converted + ".txt")
        ...
        if object_id > 0:
            try:
                point_cloud_instance = point_cloud[point_cloud[:, 7] == object_id_ori]
                ...
                prompt_object_category = " Additional information is: The class of this object in image belongs to '" + category.lower() + "'.<end> Query is: "
            except IndexError:
                prompt_object_category = " Additional information is: The class of this object in image belongs to '" + object_name + "'.<end> Query is: "
        else: 
            prompt_object_category = " Additional information is: The class of this object in image belongs to '" + object_name + "'.<end> Query is: "
        ...
        try:
            ...
            with open(file_path, "a", encoding='utf-8') as file:
                file.write( str(scene_id) + ", object_id:" + str(object_id_ori) + ", ann_id:" + str(ann_id) + ", rank:" + "999" + ", pos:" + "0" + "\n" )
            continue
        ...
        for i_2 in cand_instance_ids:
            ...
            if object_id > 0:
                try:
                    point_cloud_instance = point_cloud[point_cloud[:, 7] == object_id]
                    CategoryID = point_cloud_instance[1,8]
                    id_to_category = {v: k for k, v in Fine_grained_building_category.items()}
                    category = id_to_category.get(CategoryID, "Ordinary Building")
                    prompt_object_category = " Additional information is: The class of this object in image belongs to '" + category.lower() + "'.<end> Query is: "
        ...
        if len(list_combined) == 0:
            rank = 1
            print("[EMPTY] list_combined is empty")
            print("The rank is: ", rank)
            print("The scene is: ", scene_id)
            print("The object is: ", object_id_ori)

            with open(file_path, "a", encoding='utf-8') as file:
                file.write( str(scene_id) + ", object_id:" + str(object_id_ori) + ", ann_id:" + str(ann_id) + ", rank:" + "1" + ", pos:" + str(lang_ans_object) + "\n" )
